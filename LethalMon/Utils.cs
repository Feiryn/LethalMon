...
public class Utils
{
    ...
    public static EnemyAI? SpawnEnemyAtPosition(Enemy enemy, Vector3 position) => SpawnEnemyAtPosition(enemy.ToString(), position);

    public static EnemyAI? SpawnEnemyAtPosition(string enemyTypeName, Vector3 position)
    {
    var enemyType = EnemyTypes.Where((e) => e.name == enemyTypeName).FirstOrDefault();
    if (enemyType?.enemyPrefab == null)
    {
        LethalMon.Log("Unable to spawn enemy with name \"" + enemyTypeName + "\". Enemy prefab not found.");
        return null;
    }

    LethalMon.Logger.LogInfo("Spawn enemy: " + enemyTypeName);
